...
<main class="flex h-screen overflow-hidden">
    <aside class="grid h-full w-80 content-start gap-4 border-r bg-gray-100 p-4">
        <section>
            <input
                    placeholder="Search"
        ...
        <section class="grid gap-2">
            ...
            <ul class="grid">
                {#each lists as list}
                    <li>
                        <div
                                class="flex cursor-pointer items-center justify-between gap-4 rounded-2xl p-4 transition-all {list.title ===
							activeList.title
								? 'bg-gray-200'
								: 'bg-transparent'}"
        ...
        <section class="grid gap-2">
            ...
            <ul class="flex gap-1">
                {#each tags as tag}
                    <li>
                        <div
                                class="cursor-pointer rounded px-2.5 py-1.5 text-sm transition-all {tag.title ===
							activeList.title
								? 'bg-blue-400'
								: 'bg-gray-200'}"
